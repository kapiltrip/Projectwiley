...
\begin{figure}[tbp]
\centering
\wileyfig{01_overall_ml_timing_closure_process.png}{Timing-closure workflow comparison from \cite{ref28}: conventional multi-corner closure versus a machine-learning-assisted closure flow that executes dominant corners directly and predicts the remaining corners.}
\caption{Timing-closure workflow comparison from \cite{ref28}: conventional multi-corner closure versus a machine-learning-assisted closure flow that executes dominant corners directly and predicts the remaining corners.}
...
\begin{figure}[tbp]
\centering
\wileyfig{02_sta_illustration.jpg}{Static timing analysis illustration from \cite{ref49}. This timing-graph baseline underlies the slack, WNS, and TNS targets reused by later ML work.}
\caption{Static timing analysis illustration from \cite{ref49}. This timing-graph baseline underlies the slack, WNS, and TNS targets reused by later ML work.}
...
\begin{figure}[tbp]
\centering
\wileyfig{03_block_vs_path_traversal.jpg}{Path-based versus block-based timing graph traversal from \cite{ref49}. The trade-off between cheap pessimistic propagation and more expensive accurate path reasoning motivates both ML surrogates and GPU acceleration.}
\caption{Path-based versus block-based timing graph traversal from \cite{ref49}. The trade-off between cheap pessimistic propagation and more expensive accurate path reasoning motivates both ML surrogates and GPU acceleration.}
...
\begin{figure}[tbp]
\centering
\wileyfig{04_placement_guidance_framework.png}{Placement-optimization guidance framework from \cite{ref1}, showing GNN-driven sizing, buffering, and path grouping integrated into placement.}
\caption{Placement-optimization guidance framework from \cite{ref1}, showing GNN-driven sizing, buffering, and path grouping integrated into placement.}
...
\begin{figure}[tbp]
\centering
\wileyfig{05_preroute_timing_prediction_flow.jpg}{Pre-route timing prediction and optimization flow from \cite{ref17}, linking P\&R stages, pre-route features, training data generation, and timing prediction.}
\caption{Pre-route timing prediction and optimization flow from \cite{ref17}, linking P\&R stages, pre-route features, training data generation, and timing prediction.}
...
\begin{figure}[tbp]
\centering
\wileyfig{06_protime_endpoint_embedding.png}{Endpoint-wise multimodal embedding framework from \cite{ref8}, combining netlist and layout information for optimization-aware preroute prediction.}
\caption{Endpoint-wise multimodal embedding framework from \cite{ref8}, combining netlist and layout information for optimization-aware preroute prediction.}
\label{fig:wiley-protime}
\end{figure}

\begin{figure}[tbp]
\centering
\wileyfig{07_gr_dr_timing_consistency.jpg}{GR-to-DR timing-alignment comparison from \cite{ref10}, contrasting traditional parasitic estimation, ground-truth detailed-route extraction, and ML-based prediction.}
\caption{GR-to-DR timing-alignment comparison from \cite{ref10}, contrasting traditional parasitic estimation, ground-truth detailed-route extraction, and ML-based prediction.}
...
\begin{figure}[tbp]
\centering
\wileyfig{08_bayesian_multicorner_workflow.png}{Bayesian multicorner timing-prediction workflow from \cite{ref16}, where model prediction is guarded by feature engineering, posterior estimation, and double-checking of critical paths.}
\caption{Bayesian multicorner timing-prediction workflow from \cite{ref16}, where model prediction is guarded by feature engineering, posterior estimation, and double-checking of critical paths.}
...
\begin{figure}[tbp]
\centering
\wileyfig{09_psn_mlp_architecture.png}{MLP timing-arc architecture from \cite{ref12} for dynamic supply-noise-aware timing analysis inside STA.}
\caption{MLP timing-arc architecture from \cite{ref12} for dynamic supply-noise-aware timing analysis inside STA.}
\label{fig:wiley-psn}
\end{figure}
...
\begin{figure}[tbp]
\centering
\wileyfig{10_aging_critical_cell_pipeline.png}{Aging-aware critical-cell selection pipeline from \cite{ref23}, using structure and attribute encoders to classify cells that become critical after aging.}
\caption{Aging-aware critical-cell selection pipeline from \cite{ref23}, using structure and attribute encoders to classify cells that become critical after aging.}
\label{fig:wiley-aging-composite}
\end{figure}

\begin{figure}[tbp]
\centering
\wileyfig{11_aging_path_shift_example.png}{Aging can reorder critical paths, from \cite{ref23}. Paths that are uncritical at nominal conditions may become critical after aging and workload stress.}
\caption{Aging can reorder critical paths, from \cite{ref23}. Paths that are uncritical at nominal conditions may become critical after aging and workload stress.}
...
\begin{figure}[tbp]
\centering
\wileyfig[height=0.27\textheight]{12_gpu_sta_taskflow.png}{CPU--GPU heterogeneous multicorner STA taskflow from \cite{ref22}, showing partitioned edge-copy, table-copy, arc-copy, delay computation, and propagation.}
\caption{CPU--GPU heterogeneous multicorner STA taskflow from \cite{ref22}, showing partitioned edge-copy, table-copy, arc-copy, delay computation, and propagation.}
\label{fig:wiley-gpu-sta}
\end{figure}

\begin{figure}[tbp]
\centering
\wileyfig[trim=0 0 0 28,clip,height=0.27\textheight]{13_gpu_pba_preprocess.png}{GPU-accelerated path-based-analysis preprocessing from \cite{ref21}, illustrating path-constraint handling and rank-based graph preparation before parallel PBA kernels.}
\caption{GPU-accelerated path-based-analysis preprocessing from \cite{ref21}, illustrating path-constraint handling and rank-based graph preparation before parallel PBA kernels.}
